...
def get_user(username: str):
    ...
    with _connect() as conn:
        db = conn.cursor()
        row = db.execute(
            "SELECT username, password_hash, tokens, joined_at, usage_count FROM users WHERE username=?",
            (username,),
        ).fetchone()
        if not row:
            return None
...
def increment_usage(username: str):
    ...
    with _connect() as conn:
        db = conn.cursor()
        db.execute(
            "UPDATE users SET usage_count = usage_count + 1 WHERE username=?", (username,)
        )
        conn.commit()
